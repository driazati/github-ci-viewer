...
function try_reattach_old_display(merge_status_list) {
  ...
  if (!document.querySelector("#ci_viewer_display") && current_display) {
    ...
    current_display
      .querySelector("#circleci_viewer_freshness")
      .classList.add("tooltip");
    current_display.querySelector("#circleci_viewer_freshness").innerHTML =
      "<span class='tooltiptext'>The build status has been updated, but this log has not been</span>";
  }
}

function rerun_all_builds_click(event) {
  // Find failed builds
  let status_list_el = event.target
    .closest("div.branch-action-item")
    .querySelector(".merge-status-list");
  let builds = get_builds(status_list_el);
  builds = builds.filter(
    (build) => build.status === "failed" && build.link.includes("circleci.com")
  );
...
function add_rerun_failed_button(body) {
  ...
  let hide_all_checks = parent.querySelector(
    "button.btn-link.float-right.js-details-target"
  );
  if (!hide_all_checks) {
