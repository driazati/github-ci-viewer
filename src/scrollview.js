...
class ScrollView {
  ...
  _onScroll() {
    ...
    if (this.div.scrollTop <= 200) {
      ...
      this.div.innerHTML = "";
      this.div.appendChild(this.buildLogWithLines());
  ...
  buildLogWithLines() {
    ...
    div.classList.add("log-text");
    const lines = this.visible_text.split("\n");
    const textNumLines = (this.text.match(/\n/g) || []).length;
    const visibleNumLines = lines.length;
    for (let i = 0; i < lines.length; i++) {
      let lineNumber = document.createElement("span");
      lineNumber.innerText = `${textNumLines - visibleNumLines + i + 2} `;
      lineNumber.classList.add("line-number");
      let logLine = document.createElement("span");
      const lineDiv = document.createElement("div");
      logLine.innerText = lines[i];
      lineDiv.appendChild(lineNumber);
      lineDiv.appendChild(logLine);
