class AzureItem {
  ...
  getDisplay(callback) {
    if (this.build.specific_name === undefined) {
      let error =
        "Cannot get info for generic Azure job (try one of the other Azure jobs)";
    ...
    let success = (api_response) => {
      ...
      for (let i = 0; i < api_response.records.length; i++) {
        ...
        if (
          ...
          item.type &&
          item.type === "Job" &&
          item.name.includes(this.build.specific_name)
        ) {
          builds.push(item);
        }
      }
    ...
    let checks_success = (checks_response) => {
      ...
      this.build.id = new URL(checks_response.details_url).searchParams.get(
        "buildId"
      );
